...
open class CoroutinesCache(private val context: Context) : CoroutineScope {
    ...
    inline fun <reified T : Any> asyncCache(
        ...
        return when (cachePolicy) {
            ...
            is CachePolicy.IgnoreCache -> cacheProviderResolver(source, key, cachePolicy)
        }
    }

    @PublishedApi
    internal inline fun <reified T : Any> cacheLifeResolver(
        noinline source: suspend () -> Deferred<T>,
        key: String,
        cachePolicy: CachePolicy.LifeCache
    ): Deferred<T> {
    ...
    @PublishedApi
    internal fun <T> getFromSource(source: suspend () -> Deferred<T>, key: String): Deferred<T> {
        return async {
    ...
    @PublishedApi
    internal inline fun <reified T : Any> getFromCacheValidateTime(key: String, lifeCache: CachePolicy.LifeCache): T? {
